...
import {
    PlusCircle, MapPin, Building, Plane, Loader2, ServerCrash, Trash2, Edit, X,
    AlertTriangle, ParkingCircle, Search, ChevronLeft, ChevronRight,
} from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import toast from 'react-hot-toast';

// Komponen-komponen kecil (Header, StatCard, Modals) tidak berubah
...
// --- PERBAIKAN 1: LocationRow sekarang hanya me-render sel-sel (<td>) ---
// Komponen ini tidak lagi bertanggung jawab atas animasi atau elemen <tr>.
const LocationRow = ({ location, onEditClick, onDeleteClick }) => {
    const TypeIcon = ({ type }) => {
    ...
    return (
        <>
            ...
            <td className="px-6 py-4 text-center">{location.available_slots} / {location.total_slots}</td>
            <td className="px-6 py-4">
                <div className="flex justify-center gap-4">
                    <button onClick={() => onEditClick(location)} className="text-blue-500 hover:text-blue-700"><Edit className="w-4 h-4" /></button>
                    <button onClick={() => onDeleteClick(location)} className="text-red-500 hover:text-red-700"><Trash2 className="w-4 h-4" /></button>
                </div>
            </td>
        </>
    );
...
export default function AdminLocationsPage() {
    ...
    const fetchLocations = useCallback(async (page, search) => {
        ...
        try {
            ...
            setStats({
                ...
                availableLocationsCount: data.availableLocationsCount || 0
            });
    ...
    // Handler Create/Update/Delete (tidak ada perubahan)
    const handleSubmit = async (e) => { e.preventDefault(); setIsSubmitting(true); try { const payload = { name: newName, address: newAddress, type: newType, totalSlots: parseInt(newSlots, 10), }; const response = await fetch("/api/locations", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload), }); if (!response.ok) { const errorData = await response.json(); throw new Error(errorData.error); } toast.success("Lokasi berhasil ditambahkan!"); setNewName(""); setNewAddress(""); setNewType("MALL"); setNewSlots(100); setCurrentPage(1); setSearchTerm(""); fetchLocations(1, ""); } catch (err) { toast.error(err.message); } finally { setIsSubmitting(false); } };
    const handleEdit = async (formData) => { setIsSubmitting(true); try { const response = await fetch(`/api/locations/${modalState.data.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(formData), }); if (!response.ok) { const errorData = await response.json(); throw new Error(errorData.error); } toast.success("Perubahan berhasil disimpan!"); setModalState({ type: null, data: null }); fetchLocations(currentPage, searchTerm); } catch (err) { toast.error(err.message); } finally { setIsSubmitting(false); } };
    const handleDelete = async () => { setIsSubmitting(true); try { const response = await fetch(`/api/locations/${modalState.data.id}`, { method: "DELETE" }); if (!response.ok) { const errorData = await response.json(); throw new Error(errorData.error); } toast.success("Lokasi berhasil dihapus!"); setModalState({ type: null, data: null }); fetchLocations(currentPage, searchTerm); } catch (err) { toast.error(err.message); } finally { setIsSubmitting(false); } };

    return (
        <div className="space-y-8">
            <AdminHeader />
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <StatCard title="Total Lokasi" value={loading ? "..." : stats.totalLocations} icon={MapPin} />
                <StatCard title="Total Slot Parkir" value={loading ? "..." : stats.totalSlots} icon={ParkingCircle} />
                <StatCard title="Lokasi Tersedia" value={loading ? "..." : stats.availableLocationsCount} icon={Building} />
            </div>

            {/* Form Tambah Lokasi (tidak ada perubahan) */}
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md"> <h2 className="text-xl font-bold mb-4">Tambah Lokasi Baru</h2> <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end"> <div className="lg:col-span-2"> <label htmlFor="name" className="block text-sm font-medium mb-1">Nama Lokasi</label> <input type="text" id="name" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="e.g., Grand Indonesia" className="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md" required /> </div> <div className="lg:col-span-2"> <label htmlFor="address" className="block text-sm font-medium mb-1">Alamat</label> <input type="text" id="address" value={newAddress} onChange={(e) => setNewAddress(e.target.value)} placeholder="e.g., Jl. MH Thamrin No.1" className="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md" required /> </div> <div> <label htmlFor="type" className="block text-sm font-medium mb-1">Tipe</label> <select id="type" value={newType} onChange={(e) => setNewType(e.target.value)} className="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md"> <option>MALL</option> <option>BANDARA</option> <option>GEDUNG</option> </select> </div> <div> <label htmlFor="slots" className="block text-sm font-medium mb-1">Total Slot</label> <input type="number" id="slots" value={newSlots} onChange={(e) => setNewSlots(e.target.value)} className="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md" min="1" required /> </div> <button type="submit" disabled={isSubmitting} className="w-full flex justify-center items-center px-4 py-2 bg-blue-600 text-white font-semibold rounded-md hover:bg-blue-700 disabled:bg-blue-400"> {isSubmitting ? <Loader2 className="w-5 h-5 animate-spin" /> : <PlusCircle className="w-5 h-5 mr-2" />} {isSubmitting ? 'Menyimpan...' : 'Tambah'} </button> </form> </div>
                
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md">
                <div className="p-4 border-b dark:border-gray-700 flex justify-between items-center">
                    <h3 className="text-lg font-semibold">Daftar Lokasi ({stats.totalLocations})</h3>
                    <div className="relative">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                        <input type="text" placeholder="Cari lokasi atau tipe..." value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1); }} className="pl-10 pr-4 py-2 w-64 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md" />
                    </div>
                </div>
                ...
                : (
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody>
                                <AnimatePresence>
                                    {/* --- PERBAIKAN 2: Logika animasi dipindahkan ke sini --- */}
                                    {/* Kita membungkus LocationRow dengan motion.tr */}
                ...
                {totalPages > 1 && (
                    <div className="p-4 border-t dark:border-gray-700 flex justify-between items-center">
                        <span className="text-sm text-gray-500 dark:text-gray-400">
                            Menampilkan {locations.length} dari {stats.totalLocations} lokasi
                        </span>
