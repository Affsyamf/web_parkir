...
const AdminHeader = () => (
  <div className="mb-8">
    ...
    <p className="text-gray-500 dark:text-gray-400">
      Kelola semua lokasi parkir di sini.
    </p>
  </div>
);
  ...
  <>
        ...
        <div className="ml-4 text-left">
          ...
          <div className="mt-2">
            <p className="text-sm text-gray-500 dark:text-gray-400">
              ...
              <strong>{location?.name}</strong>? Tindakan ini tidak dapat
              diurungkan. Semua slot parkir yang terkait juga akan dihapus.
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="bg-gray-50 dark:bg-gray-700/50 px-6 py-4 flex justify-end gap-3 rounded-b-lg">
      <button
        onClick={onCancel}
        type="button"
        className="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-gray-900"
      >
        Batal
      </button>
      <button
        ...
        disabled={isDeleting}
        type="button"
        className="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-red-600 border border-transparent rounded-md hover:bg-red-700 disabled:bg-red-400"
      >
...
const EditLocationModal = ({ location, onSave, onCancel, isSaving }) => {
...
// --- PERBAIKAN: Komponen LocationRow dipindahkan ke luar ---
...
export default function AdminLocationsPage() {
  ...
  const handleEdit = async (formData) => {
    setIsSubmitting(true);
    try {
      const response = await fetch(`/api/locations/${modalState.data.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const totalSlots = loading
    ? "..."
    : (locations || []).reduce((sum, loc) => sum + (loc.total_slots || 0), 0);
  const availableLocations = loading
    ? "..."
    : (locations || []).filter((loc) => (loc.available_slots || 0) > 0).length;
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
        ...
        >
          ...
          <button
            ...
            className="w-full flex justify-center items-center px-4 py-2 bg-blue-600 text-white font-semibold rounded-md hover:bg-blue-700 disabled:bg-blue-400 disabled:cursor-not-allowed transition-colors"
          >
            {isSubmitting ? (
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md">
        <div className="p-4 border-b dark:border-gray-700">
          <h3 className="text-lg font-semibold">
            Daftar Lokasi ({!loading && locations ? locations.length : 0})
          </h3>
        </div>
